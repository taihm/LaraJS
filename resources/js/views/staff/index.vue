    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'StaffCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.staff.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="building_id" prop="building_id" :label="$t('table.staff.building_id')" align="center" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.building_id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="user_id" prop="user_id" :label="$t('table.staff.user_id')" align="center" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.user_id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="staff_position_id" prop="staff_position_id" :label="$t('table.staff.staff_position_id')" align="center" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.staff_position_id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="status" prop="status" :label="$t('table.staff.status')" align="center" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.status }}
                </template>
              </el-table-column>
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :title="$t('button.edit')" :to="{name: 'StaffEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await staffResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.staff.id') + '#' + id }), this.$t('messages.warning'), {
          ...
          try {
            this.table.loading = true;
            await staffResource.destroy(id);
