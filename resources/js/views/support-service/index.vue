    ...
    <el-col :span="24">
      <el-card>
        <div slot="header" class="tw-flex tw-justify-end tw-items-center">
          <router-link v-slot="{ href, navigate }" v-permission="['create']" :to="{name: 'SupportServiceCreate'}" custom>
        ...
        <el-row :gutter="20">
          <el-col :span="24" class="table-responsive">
            ...
            >
              <el-table-column align="center" sortable="custom" prop="id" :label="$t('table.support_service.id')" width="70px">
                <template slot-scope="{ row }">
                  {{ row.id }}
                </template>
              </el-table-column>
              <el-table-column data-generator="name" prop="name" :label="$t('table.support_service.name')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.name }}
                </template>
              </el-table-column>
              <el-table-column data-generator="description" prop="description" :label="$t('table.support_service.description')" align="left" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.description }}
                </template>
              </el-table-column>
              <el-table-column data-generator="fee" prop="fee" :label="$t('table.support_service.fee')" align="center" header-align="center">
                <template slot-scope="{ row }">
                  {{ row.fee }}
                </template>
              </el-table-column>
              ...
              <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{ row }">
                  <router-link v-permission="['edit']" :title="$t('button.edit')" :to="{name: 'SupportServiceEdit', params: {id: row.id}}"><i class="el-icon-edit el-link el-link--primary tw-mr-2" /></router-link>
...
<script>
  ...
  import SupportServicesResource from '@/api/v1/support-service';
  const supportServiceResource = new SupportServicesResource();
    ...
    methods: {
      async getList() {
        try {
          ...
          const { data } = await supportServiceResource.list(this.table.listQuery);
      ...
      remove(id) {
        this.$confirm(this.$t('messages.delete_confirm', { attribute: this.$t('table.support_service.id') + '#' + id }), this.$t('messages.warning'), {
          ...
          try {
            this.table.loading = true;
            await supportServiceResource.destroy(id);
